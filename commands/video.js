...
async function handleYtVideoReply(sock, chatId, message) {
    try {
        ...
        // Direct yt-dlp command ( Audio + Video Merge )
        const ytDlpCmd = `yt-dlp -f "bv*[height<=${targetHeight}][ext=mp4]+ba[ext=m4a]/b[height<=${targetHeight}]" --merge-output-format mp4 -o "${outputPath}" "${downloadData.url}"`;
...
module.exports = { videoCommand, handleYtVideoReply };
