...
async function handleYtVideoReply(sock, chatId, message) {
    try {
        ...
        // Direct yt-dlp command with --cookies cookies.txt
        const ytDlpCmd = `yt-dlp --cookies cookies.txt -f "bv*[height<=${targetHeight}][ext=mp4]+ba[ext=m4a]/b[height<=${targetHeight}]" --merge-output-format mp4 -o "${outputPath}" "${downloadData.url}"`;
