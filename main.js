...
const { helpCommand, handleMenuReply } = require('./commands/help');
...
const { nsfwMenuCommand, handleNsfwImage, nsfwTypes } = require('./commands/nsfw');
const warnCommand = require('./commands/warn');
const warningsCommand = require('./commands/warnings');
const ttsCommand = require('./commands/tts');
const { cosplayMenuCommand, handleCosplayImage } = require('./commands/cosplay');
...
const { facebookCommand, handleFbReply } = require('./commands/facebook');
...
async function handleMessages(sock, messageUpdate, printLog) {
    ...
    try {
        ...
        // TikTok reply and menu handling
        const isTikTokReply = await tiktokCommand(sock, chatId, message);
        if (isTikTokReply) return;

        // Video selection reply handler (1, 2, 3 or document options)
        const isVideoHandled = await handleVideoReply(sock, chatId, message);
        if (isVideoHandled) return;

       // Menu Quoted Reply Detection (1-13)
        const contextInfo = message.message?.extendedTextMessage?.contextInfo;
        const quotedMsg = contextInfo?.quotedMessage;
        
        const quotedMsgText = quotedMsg?.conversation ||
                             quotedMsg?.extendedTextMessage?.text ||
                             quotedMsg?.imageMessage?.caption ||
                             quotedMsg?.videoMessage?.caption || "";

        if (quotedMsgText.includes("X BOT COMMAND MENU") || quotedMsgText.includes("Type 1 for General Commands")) {
            const isHandled = await handleMenuReply(sock, chatId, message);
            if (isHandled) return;
        }
       // Handle FB Quality Choice Reply
        ...
        switch (true) {
            ...
            case userMessage.startsWith('.video') || userMessage.startsWith('.ytmp4'):
                ...
                break;
            case userMessage === '.cosplay':
                await cosplayMenuCommand(sock, chatId, message);
                break;
            case ['.asuma', '.ayaka', '.furina', '.ganyu', '.kurumi', '.nahida', '.yaemiko', '.yume'].includes(userMessage): {
                const character = userMessage.slice(1).toLowerCase();
                await handleCosplayImage(sock, chatId, message, character);
                break;
            }
            case userMessage === '.nsfw':
                await nsfwMenuCommand(sock, chatId, message);
                break;
            case userMessage.startsWith('.') && nsfwTypes.includes(userMessage.slice(1).toLowerCase()): {
                const type = userMessage.slice(1).toLowerCase();
                await handleNsfwImage(sock, chatId, message, type);
                break;
            }     
